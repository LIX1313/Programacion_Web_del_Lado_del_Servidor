...
// Archivos estáticos
app.use(express.static("public")); // Permite acceder a archivos estáticos desde la carpeta public
...
app.get("/", async (req, res) => {
  try {
    ...
    const result = await axios.get(
      "https://yurippe.vercel.app/api/quotes?random=5",
    );
    // Extracción de datos de la respuesta
    const quotes = result.data; // Todo el array

    // Renderizado de la vista index.ejs con datos dinámicos
    res.render("index", {
      quotes: quotes // Se envía la cita a la vista
    });
